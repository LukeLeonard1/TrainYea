...
public class Train {
	...
	public String toString() {
		...
		for (int i = 1; i<= trains.size();i++) {
			temp += " Cart "+ i + " (" + trains.get(i-1).seatRange() + ")" + 
		"   (Random used :" + Math.floor((trains.get(i-1).getCurrentCapacity()*trains.get(i-1).KG_PER_PERSON + trains.get(i-1).TRAIN_WEIGHT)) + ")";//cart range
		}
